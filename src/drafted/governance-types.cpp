...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "BlasNetwork") type = BlasNetwork;
    if(strType == "BlasNetworkVariable") type = BlasNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == BlasNetwork) s = "BlasNetwork";
    if(type == BlasNetworkVariable) s = "BlasNetworkVariable";
