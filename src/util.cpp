...
const char * const BITCOIN_CONF_FILENAME = "blas.conf";
const char * const BITCOIN_PID_FILENAME = "blasd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "blas" is a composite category enabling all Blas-related debug output
            if(ptrCategory->count(string("blas"))) {
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BlakeStar";
#else
    // Unix
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    if (!boost::filesystem::exists(GetConfigFile())){
        // Create empty blas.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override blas.conf
