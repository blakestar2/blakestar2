...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %s BLAS, masternode=%s\n", MASTERNODE_COLLATERAL_AMOUNT, vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 13750 BLAS tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 13750 BLAS tx -> 1 confirmation
