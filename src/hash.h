...
/* ----------- Blas Hash ------------------------------------------------ */
template<typename T1>
inline uint256 HashX22(const T1 pbegin, const T1 pend)
{
    ...
    sph_sha512_init(&ctx_sha2);
    sph_sha512 (&ctx_sha2, static_cast<const void*>(&hash[18]), 64);
    sph_sha512_close(&ctx_sha2, static_cast<void*>(&hash[19]));

    sph_haval256_5_init(&ctx_haval);
    sph_haval256_5 (&ctx_haval, static_cast<const void*>(&hash[19]), 64);
    sph_haval256_5_close(&ctx_haval, static_cast<void*>(&hash[20]));

    sph_whirlpool_init(&ctx_whirlpool);
    sph_whirlpool (&ctx_whirlpool, static_cast<const void*>(&hash[20]), 64);
    sph_whirlpool_close(&ctx_whirlpool, static_cast<void*>(&hash[21]));
