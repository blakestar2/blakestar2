...
*Setup instructions for a Gitian build of Blas Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Blas
...
- [Building Blas Core](#building-blas-core)
...
Clone the git repositories for Blas Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/blakestar2/blakestar2
```
...
in the Blas Core repository under 'Fetch and build inputs' to install sources which require
...
To build Blas Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Blas Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/blas/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/blakestar2/blakestar2
...
For example:
```bash
URL=https://github.com/crowning-/blas.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit blas=${COMMIT} --url blas=${URL} ../blas/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit blas=${COMMIT} --url blas=${URL} ../blas/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit blas=${COMMIT} --url blas=${URL} ../blas/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the blas git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../blas/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/blas.git
SIGPATH=/some/root/path/blas-detached-sigs.git

./bin/gbuild --url blas=${BTCPATH},signature=${SIGPATH} ../blas/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/blas-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/blas-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/blas-osx-build.assert
```
...
[blas/gitian.sigs](https://github.com/blakestar/gitian.sigs/) repository, or if that's not possible to create a pull
request.
